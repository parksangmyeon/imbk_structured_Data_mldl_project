...
추천 결과는 `K=1`, `K=2`, `K=3`, `K=5` 기준으로 함께 확인했습니다. 성능표에서는 K가 커질수록 HitRate와 Recall이 전반적으로 높아졌지만, K=5에서는 Random 방식조차 HitRate가 약 0.9 수준까지 올라가 모델 간 변별력이 크게 약해졌습니다. 즉 K=5는 “추천을 잘했다”기보다 후보를 많이 제시해서 맞힐 가능성이 커진 측면이 강했습니다.

| K | 해석 | 판단 |
|---:|---|---|
| 1 | 고객당 1개 상품만 추천 | 추천 집중도는 높지만 실제 신규보유 상품을 놓칠 가능성이 큼 |
| 2 | 고객당 2개 상품 추천 | K=1보다 안정적이지만 후보 폭이 여전히 좁음 |
| 3 | 고객당 3개 상품 추천 | 모델 간 성능 차이가 유지되면서도 추천 후보 수가 과도하지 않음 |
| 5 | 고객당 5개 상품 추천 | HitRate는 가장 높지만 Random도 함께 높아져 평가 변별력이 낮아짐 |

따라서 최종 보고 기준은 `K=3`으로 설정했습니다. K=3은 추천 후보 수가 과도하게 늘어나지 않으면서도 Item-CF, Popularity, Random 및 Hybrid 모델 간 성능 차이를 비교할 수 있는 기준이었기 때문입니다.
